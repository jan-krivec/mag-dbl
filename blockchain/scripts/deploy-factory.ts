...
async function main() {
  ...
  // const factory = new ethers.Contract("0xE96A9Aa963C4f5BCA028ca1E022F99e8BCED6db3", jsonFactory.abi, deployer);
  ...
  // const identityAddress = await factory.getIdentity("0x8a0d5408cCCe5F6d7496515C960Aa6D83c352651");
  // console.log(identityAddress);

  for (const i in contractAdresses) {
    const addr = contractAdresses[i];
    const tx = await factory.createIdentity(addr, addr);

    console.log(`Deploy a new identity as a proxy using factory ${factory.address} . tx: ${tx.hash}`);

    await tx.wait();
    const identityAddress = await factory.getIdentity(addr);

    console.log(`Deployed a new identity for ca ${addr} at ${identityAddress} as a proxy using factory. tx: ${tx.hash}`);
  }
}
